      ...
      return (
        <div style={{ padding: 32, fontFamily: "monospace" }}>
          ...
          <button onClick={() => { localStorage.clear(); location.reload(); }}
            style={{ marginTop: 16, padding: "8px 16px", background: "#e55", color: "#fff", border: "none", borderRadius: 6, cursor: "pointer" }}>
            ストレージをクリアして再起動
          </button>
        </div>
      );
...
function TopNav({ user, onLogout }: { user: UserProfile; onLogout: () => void }) {
    ...
    <div className="sticky top-0 z-40 w-full backdrop-blur-md bg-white/80 border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-3">
            ...
            <div>
                {/* Updated Branding */}
                <h1 className="font-semibold text-slate-800 tracking-tight leading-none">Wisteria Group</h1>
                <div className="text-[10px] text-slate-500 uppercase tracking-widest">アンケート エンジン</div>
            </div>
        </div>
        ...
        <div className="flex items-center gap-4">
             ...
             <button onClick={onLogout} className="text-slate-400 hover:text-emerald-600 transition-colors">
